 ...
 */
    ...
    if($Content->updateAllUrl()) {
        $this->setUpdateLog(__d('baser_dev','contents テーブルのデータ更新に成功しました。')); // CHECK
    } else {
        $this->setUpdateLog(__d('baser_dev','contents テーブルのデータ更新に失敗しました。'), true); // CHECK
    }
 ...
 */
    if($this->loadSchema('4.0.1', '', 'sites', $filterType = 'alter')) {
        $this->setUpdateLog(__d('baser_dev','sites テーブルの構造変更に成功しました。')); // CHECK
    } else {
        $this->setUpdateLog(__d('baser_dev','sites テーブルの構造変更に失敗しました。'), true); // CHECK
    }
